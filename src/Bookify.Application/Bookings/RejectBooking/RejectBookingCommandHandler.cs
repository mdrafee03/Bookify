...
{
    private readonly ILogger<RejectBookingCommandHandler> _logger = logger;
    ...
    {
        _logger.LogInformation("Rejecting booking with id {Id}", request.BookingId);
        ...
        {
            _logger.LogWarning(
                "User {UserId} is not authorized to reject booking with id {Id}",
                _userContext.UserId,
                request.BookingId
            );
        ...
        _logger.LogInformation("Booking with id {Id} rejected", request.BookingId);
